...
// GET All
router.get('/', (req, res) => {
  db.all("SELECT * FROM movements ORDER BY date ASC", (err, rows) => {
...
router.post('/bulk-delete', async (req, res) => {
  ...
  try {
    ...
    const result = await dbRun(`DELETE FROM movements WHERE id IN (${placeholders})`, ids);
    // @ts-ignore
    res.json({ success: true, deletedCount: result.changes });
    addLogEntry('INFO', `Bulk delete: ${ids.length} Datensätze gelöscht.`);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post('/batch', async (req, res) => {
  ...
  try {
    // 1. Immediate Transaktion starten
    ...
    for (const m of movements) {
      await new Promise((resolve, reject) => {
        stmt.run(
          ...
          m.durationNetto || 0, m.amount || 0, m.isManual ? 1 : 0,
          (err) => {
            if (err) reject(err);
            else resolve(true);
          }
