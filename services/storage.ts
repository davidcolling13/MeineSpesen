...
export const bulkDeleteMovements = async (ids: string[]) => {
  ...
  if (!isValidApiResponse(res)) throw new Error("Bulk-Löschen fehlgeschlagen: Server Fehler");
  
  // Cache Update
  const idSet = new Set(ids);
  const current = localGet<Movement[]>(STORAGE_KEYS.MOVEMENTS, []);
  localSet(STORAGE_KEYS.MOVEMENTS, current.filter(m => !idSet.has(m.id)));
};
