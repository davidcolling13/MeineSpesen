...
const ReportView: React.FC = () => {
  ...
  const openPdfInNewTab = (blob: Blob, fallbackFileName: string) => {
    const url = URL.createObjectURL(blob);
    const win = window.open(url, '_blank');
    if (!win) {
      FileSaver.saveAs(blob, fallbackFileName);
    }
  ...
  const handleBulkPrint = async () => {
      ...
      try {
          ...
          const fileName = `Spesen_Gesamt_${selectedYear}-${selectedMonth+1}.pdf`;
          openPdfInNewTab(blob, fileName);
  ...
  const handlePrintOverview = async () => {
      ...
      try {
          ...
          const fileName = `Spesen_Uebersicht_${selectedYear}-${selectedMonth+1}.pdf`;
          openPdfInNewTab(blob, fileName);
